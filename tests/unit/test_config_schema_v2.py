"""Validation of the v3 config schema fields: schema_version, sortable.

(File retains its v2 history-name; tests now exercise v3 shape since the
v2 schema is replaced. The schema_version check and sortable-defaulting
behavior carry over directly.)"""
...
def test_sortable_defaults_to_true_when_omitted():
    cfg, errors = validate_config_dict(_with_ratings([
        {"id": "x", "label": "X", "enabled": True, "weight": 1.0},
    ]))
    assert errors == []
...
def test_sortable_explicit_false_is_preserved():
    cfg, errors = validate_config_dict(_with_ratings([
        {"id": "x", "label": "X", "enabled": True, "sortable": False, "weight": 1.0},
    ]))
    assert errors == []
